...
// Regla: para el mismo id gana updatedAt más reciente. Una copia inicial seed
// nunca reemplaza un registro que ya fue creado o modificado realmente.
(()=>{
  ...
  async function mergeLatest(cloud){
    ...
    for(const store of stores){
      const local=await all(store),remote=Array.isArray(cloud?.[store])?cloud[store]:[],map=new Map();
  ...
  async function syncLatest(){
    ...
    try{
      b.disabled=true;b.textContent='☁ Sincronizando…';if(s)s.textContent='Comparando cambios con Google Drive…';
      ...
      await writeMergedLocal(merged);await uploadCloud(id,merged);await seedParts();await refresh();
      ...
      location.reload();
    }catch(e){console.error('SK Web sync v2:',e);if(s)s.textContent='Error de sincronización: '+(e?.message||e);alert('No se pudo sincronizar: '+(e?.message||e))}
