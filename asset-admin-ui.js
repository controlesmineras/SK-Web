...
function updateAssetFields(){const f=document.querySelector('#assetForm');if(!f)return;const largo=f.elements.largo?.closest('label');if(largo)largo.hidden=(f.elements.clase?.value||'').trim().toLowerCase()!=='columna'}
...
window.addEventListener('load',()=>{
  ...
  list?.addEventListener('click',e=>{if(e.target.closest('[data-id]'))setTimeout(()=>{setAssetMode(true);updateAssetFields()},0)});
  ...
  form?.addEventListener('reset',()=>setTimeout(()=>{setAssetMode(false);updateAssetFields()},0));
  setAssetMode(!!form?.elements?.id?.value);updateAssetFields();
});
})();