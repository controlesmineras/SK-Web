...
// No altera propiedad ni borra registros; solo clasifica/ordena la vista administrativa.
(()=>{
...
const compare=(a,b)=>{
  const pa=isSK(a)?0:1,pb=isSK(b)?0:1;
  if(pa!==pb)return pa-pb;
  return label(a).localeCompare(label(b),'es',{numeric:true,sensitivity:'base'});
};
function render(rows){const box=document.querySelector('#assetAdminList');if(!box)return;const q=norm(document.querySelector('#assetSearch')?.value);const filtered=rows.filter(a=>!q||norm(Object.values(a).join(' ')).includes(q)).sort(compare);box.innerHTML=filtered.map(a=>`<button class="assetRow" data-id="${a.id}"><b>${label(a)}</b><small>${a.ubicacion||'Sin ubicación'} · ${a.estado||'Sin estado'}${isSK(a)?'':` · Externo · ${a.mina||'Otra mina'}`}</small></button>`).join('')||'<p class="hint">No hay activos que coincidan.</p>'}
