// Compatibilidad de campos de activos v1.
// Mantiene la interfaz actual, pero separa internamente el número de clase de la marcación física.
(()=>{
...
function canonical(a){
  ...
  // Los YT importados originalmente traían su número normal en orden/marcaAnterior.
  if(yt){
    if(!isNum(numero)) numero=isNum(orden)?orden:(isNum(equivalencia)?equivalencia:'');
    // Si por un cruce anterior el romano cayó en numeroYT, se conserva como marca física.
    if(!marca && a.numeroYT && romanToInt(a.numeroYT)) marca=clean(a.numeroYT);
    // Si existe una marca romana, su equivalencia numérica no reemplaza el número de clase.
  }
...
async function migrate(){
  ...
  for(const a of rows){
    ...
    // Espejos de compatibilidad: la pantalla sigue usando los nombres antiguos.
